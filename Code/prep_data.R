...
##  Note: Email Terry McDonnell (tmcdonn2@nd.edu) to obtain the raw survey data
...
#Read in survey data
# qual <- read.csv(here::here("Data", "iconicity_qual.csv")) %>% #NOTE: Email
#                                                         #Terry McDonnell to obtain
#                                                         #this data file
#   slice(., -(1:2)) #Remove metadata in first two rows

#Read in the mturk data, so we can remove cases we rejected 
# mturk <- read.csv(here::here("Data", "mturk.csv")) %>% #NOTE: Email Terry McDonnell to 
#                                                   #obtain this data file
...
#filter out these 83 mistake people
icon.data.sub <- icon.data.sub %>% 
