...
icon_data_sub <- readRDS(here::here('Data', 'icon_data_sub.rds'))

  
  

# subset and create var list ----------------------------------------------
image.sub.top <- icon_data_sub %>% 
...
saveRDS(image.sub.edges, file = "iconic_ranking_edges_top_20230612.rds")
saveRDS(link_sim_mat, file = "similarity_matrix_20230612.rds")
...
saveRDS(top_link_mat_comb, file = "link_mat_top_comb_20230612.rds")

# for (i in 1:nrow(e)){
#   for (j in 1:nrow(e)){
#     r1 <- e[i,1]  #where 1 is the id corresponding to the row id of x
#     r2 <- e[j,1]  #where 1 is the id corresponding to the row id of x
#     w[i, j] <- Jaccard(x[r1,], x[r2,])
#   }
# }
